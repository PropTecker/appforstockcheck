# app.py — BNG Optimiser (Standalone), v9.6 (map-stable)
# Policy summary:
# - Always minimise total cost. No proximity preference.
# - ≤ 2 banks overall per quote.
# - Exactly one option per demand line (no splitting) EXCEPT Medium FAR 50/50 Orchard+Scrub (single option).
# - Low: legal substitutions allowed, but pricing must be from actual price rows (no group-proxies).
# - Medium: Local/Adjacent => single-habitat only; FAR may use Orchard+Scrub pair (if present/cheaper).
# - High/Very High: like-for-like only.
# - Hedgerow excluded.
# - Min line quantity = 0.01.
# - Tie-break when total cost equal: prefer banks with more total stock (tiny epsilon).
...
def geocode_address(addr: str) -> Tuple[float, float]:
    ...
    js = safe_json(http_get("https://photon.komoot.io/api/", params={"q": sstr(addr), "limit": 1}))
...
with st.sidebar:
    ...
    if not uploaded: st.info("Or use an example backend in ./data", icon="ℹ️")
    ...
    # Optional quick control to recover base map after a run
    if st.button("Reset results map view"):
        st.session_state["has_results_map"] = False
        st.rerun()
...
# ===== Map builders (with base/results guard) =====
def render_base_map(with_controls: bool = True):
    # If no coords yet show UK-wide
    if target_lat is None or target_lon is None:
        fmap = folium.Map(location=[54.5, -2.5], zoom_start=5, control_scale=True)
        if with_controls: folium.LayerControl(collapsed=True).add_to(fmap)
        return fmap
...
# Show base map only if we haven't shown a results map yet
if (target_lat is not None) and (target_lon is not None) and not st.session_state.get("has_results_map", False):
    st.markdown("### Map")
    st_folium(render_base_map(with_controls=True), height=420, returned_objects=[], use_container_width=True, key="basemap")
...
HAB_CHOICES = sorted([sstr(x) for x in backend["HabitatCatalog"]["habitat_name"].dropna().unique().tolist()] + [NET_GAIN_LABEL])
    ...
    with cc1:
        if st.button("➕ Add habitat", key="add_hab_btn"):
            st.session_state.demand_rows.append({"id": st.session_state._next_row_id,
                                                 "habitat_name": HAB_CHOICES[0] if HAB_CHOICES else "", "units": 0.0})
            st.session_state._next_row_id += 1; st.rerun()
    with cc2:
        if st.button("➕ Net Gain (Low-equivalent)", key="add_ng_btn",
                     help="Trades like Low distinctiveness (can source from any habitat)."):
            st.session_state.demand_rows.append({"id": st.session_state._next_row_id, "habitat_name": NET_GAIN_LABEL, "units": 0.0})
            st.session_state._next_row_id += 1; st.rerun()
    with cc3:
...
def enforce_catalog_rules_official(demand_row, supply_row, dist_levels_map_local, explicit_rule: bool) -> bool:
    ...
    if dh == NET_GAIN_LABEL: return True  # Net Gain behaves as Low legally
...
def prepare_options(demand_df: pd.DataFrame,
    ...
    # Effective group/distinctiveness
    ...
    def find_price_for_supply(bank_key: str,
        ...
        # Low / Net Gain: no proxy
        if sstr(demand_dist).lower() == "low":
            return None

        # Medium: proxy allowed
        if sstr(demand_dist).lower() == "medium":
    ...
    for di, drow in demand_df.iterrows():
        ...
        # Candidate stock by legality (plus explicit trading if provided)
        cand_parts = []
        explicit = False
        ...
        # Paired Orchard+Scrub for MEDIUM — FAR only
            ...
            for bk in banks_keys:
                ...
                for _, o in orch_rows.iterrows():
                    for _, s2 in scrub_rows.iterrows():
                        tier_b = tier_for_bank(sstr(s2.get("lpa_name")), sstr(s2.get("nca_name")),
                                               target_lpa, target_nca, lpa_neigh, nca_neigh, lpa_neigh_norm, nca_neigh_norm)
...
# ========= Optimiser (one option per demand; ≤2 banks overall) =========
def optimise(demand_df: pd.DataFrame,
    ...
    options, stock_caps, stock_bankkey = prepare_options(
        demand_df, chosen_size, target_lpa, target_nca, lpa_neigh, nca_neigh, lpa_neigh_norm, nca_neigh_norm
    )
    if not options:
    ...
    # MILP minimise total cost, exact one option per demand, ≤2 banks
    try:
        import pulp
        ...
        used_banks: List[str] = []
        def bank_ok(b):
            cand = set(used_banks); cand.add(b); return len(cand) <= 2
        rows = []; total_cost = 0.0
        for di, drow in demand_df.iterrows():
            need = dem_need[di]
            ...
            for i in cand_idx:
                ...
                if this_cost < best_cost - 1e-9:
                    best_cost, best_i = this_cost, i
            if best_i is None:
                name = sstr(drow["habitat_name"])
                raise RuntimeError(f"Greedy fallback infeasible for '{name}' (no single option can cover need).")
    ...
    try:
        if demand_df.empty:
            ...
            options_preview, _, _ = prepare_options(
                dd, chosen_size, sstr(target_lpa_name), sstr(target_nca_name),
            ...
            if not options_preview:
                ...
                if "price_source" in cand_df.columns:
                    st.caption("Note: `price_source='group-proxy'` means priced via group/distinctiveness rows (Medium only).")
...
def _build_pricing_enriched_for_size(chosen_size: str) -> pd.DataFrame:
    ...
    pc_join["distinctiveness_name_eff"] = np.where(pc_join["distinctiveness_name"].astype(str).str.len()>0,
                                                   pc_join["distinctiveness_name"], pc_join["distinctiveness_name_cat"])
    for c in ["broader_type_eff","distinctiveness_name_eff","tier","bank_id","habitat_name","BANK_KEY","bank_name"]:
        if c in pc_join.columns: pc_join[c] = pc_join[c].astype(str).str.strip()
    cols = ["BANK_KEY","bank_name","bank_id","contract_size","tier","habitat_name","price","broader_type_eff","distinctiveness_name_eff"]
    for c in cols:
        if c not in pc_join.columns: pc_join[c] = ""
        ...
        if prn.empty:
            ...
            try:
                ...
                if not prn_scrub.empty:
                    ...
                    st.dataframe(prn_scrub.sort_values(["tier","price","BANK_KEY","habitat_name"]),
                                 use_container_width=True, hide_index=True)
            except Exception:
                pass
            csv_bytes = prn.to_csv(index=False).encode("utf-8")
            st.download_button("Download pricing (normalised, this size) CSV",
                               data=csv_bytes, file_name=f"pricing_normalised_{chosen_size}.csv", mime="text/csv")
    except Exception as e:
...
with st.expander("💷 Pricing completeness (this contract size)", expanded=False):
    try:
        if demand_df.empty:
            ...
            chosen_size = select_contract_size(total_units, present_sizes)
            pr = backend["Pricing"].copy(); pr = pr[pr["contract_size"] == chosen_size]
            needed = pd.MultiIndex.from_product(
                [backend["Stock"]["bank_id"].dropna().unique(), demand_df["habitat_name"].unique(), ["local","adjacent","far"]],
            ...
            if missing.empty:
                ...
                st.warning("Some exact pricing rows are missing — that’s fine if untradeable or Medium uses proxies.")
                st.dataframe(missing.sort_values(["habitat_name","bank_id","tier"]),
                             use_container_width=True, hide_index=True)
    except Exception as e:
...
if run:
    try:
        if demand_df.empty:
            st.error("Add at least one demand row before optimising."); st.stop()

        # Auto-locate if user typed address/postcode but forgot to click Locate
        if not sstr(target_lpa_name) or not sstr(target_nca_name):
            if sstr(postcode) or sstr(address):
                try:
                    (_t_lpa, _t_nca, _lpaN, _ncaN, _lpaNn, _ncaNn, _lat, _lon, _lpa_gj, _nca_gj) = find_site(postcode, address)
                except Exception as e:
        ...
        alloc_df, total_cost, size = optimise(
            demand_df, target_lpa, target_nca,
            [sstr(n) for n in lpa_neighbors], [sstr(n) for n in nca_neighbors],
            lpa_neighbors_norm, nca_neighbors_norm
        )
        ...
        if "price_source" in alloc_df.columns:
            st.caption("Note: `price_source='group-proxy'` were priced using group/distinctiveness rows (Medium only).")

        st.markdown("#### By bank")
        by_bank = alloc_df.groupby(["BANK_KEY","bank_name","bank_id"], as_index=False).agg(
            units_supplied=("units_supplied","sum"), cost=("cost","sum")
        ...
        st.session_state["has_results_map"] = True  # prevent base-map from re-rendering over results

        fmap = render_base_map(with_controls=False)  # add controls after layers

        # Bank coordinates via Banks (geocode if needed)
        bank_coords: Dict[str, Tuple[float,float]] = {}
        for _, b in backend["Banks"].copy().iterrows():
                ...
                try:
                    ...
                    if cache_key not in bank_catch_cache:
                        try:
                            ...
                            bank_catch_cache[cache_key] = {"lpa_name": b_lpa_name, "lpa_gj": b_lpa_gj,
        ...
        # Add a single LayerControl once all layers are on the map
        ...
        try:
            ...
            st.warning(f"Map display issue: {_map_e}")
